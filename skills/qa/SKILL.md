...
Locate a `browse` binary in the project or environment. If your project has a custom browse tool, use it. Otherwise, ensure a headless browser tool is available for screenshot and interaction commands.
...
If user picks H → write `.no-test-bootstrap` and continue without tests.
...
1. **Conversation context:** Check if a prior architecture or engineering review produced test plan output in this conversation
2. **Use whichever source is richer.** Fall back to git diff analysis only if no richer source is available.
...
Write the report to the project's `qa-reports/` directory:
