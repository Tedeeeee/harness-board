...
find . -maxdepth 3 -name "*design*.md" -o -name "*architecture*.md" 2>/dev/null | head -5
```
If a design doc exists, read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design — check the prior version for context on what changed and why.

## Prerequisite Skill Offer

When the design doc check above finds no design doc, offer the prerequisite
...
Read the `/office-hours` skill file and follow its instructions.
...
This file is consumed by `/qa` as primary test input. Include only the information that helps a QA tester know **what to test and where** — not implementation details.
...
Analyze the plan's implementation steps for parallel execution opportunities. This helps the user split work across git worktrees or parallel workspaces.
