...
VERSION/CHANGELOG check, TODOS) runs on every invocation.
...
4. Check review readiness by examining recent git history and any prior review artifacts in the project.
...
Detect the project's test command from CLAUDE.md or auto-detect from the project structure, then run the test suite.
...
- Switch to investigation mindset: root cause first, then minimal fix.
...
```bash
bin/test-lane --eval test/evals/<suite>_eval_test.rb 2>&1 | tee /tmp/ship_evals.txt
```
...
Using the coverage percentage from the diagram in substep 4:
...
1. **Conversation context (primary):** Check if there is an active plan file in this conversation. If found, use it directly — this is the most reliable signal.
...
Automatically verify the plan's testing/verification steps.
...
Run the project test suite and verify all tests pass. Use the plan's verification section as the primary test input — treat each verification item as a test case. Use the detected dev server URL as the base URL. Cap at the verification items from the plan — do not expand into general site QA.

### 4. Gate logic

- **All verification items PASS:** Continue silently. "Plan verification: PASS."
- **Any FAIL:** Use AskUserQuestion:
  - Show the failures with evidence
...
1. Read `.claude/skills/review/checklist.md`. If the file cannot be read, use general code review best practices (security, performance, error handling, data safety).
...
```bash
# Detect frontend file changes
git diff origin/<base> --name-only | grep -E '\.(tsx?|jsx?|vue|svelte|css|scss|less|html)$' | head -5
SCOPE_FRONTEND=$( [ -n "$(git diff origin/<base> --name-only | grep -E '\.(tsx?|jsx?|vue|svelte|css|scss|less|html)$')" ] && echo "true" || echo "false" )
...
2. **Read `.claude/skills/review/design-checklist.md` if available.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
## Step 3.55: Review — Specialist Dispatch
...
1. **Testing** — review test coverage and quality
2. **Maintainability** — review code maintainability
...
3. **Security** — if auth-related files changed, OR if backend files changed AND DIFF_LINES > 100
4. **Performance** — if backend or frontend files changed
5. **Data Migration** — if migration files changed
6. **API Contract** — if API-related files changed
...
"You are a specialist code reviewer focusing on {specialist domain}. Run
`git diff origin/<base>` to get the full diff. Apply your specialist lens against the diff.
...
1. The merged specialist findings from Step 3.56 (so it knows what was already caught)
2. The git diff command

Prompt: "You are a red team reviewer. The code has already been reviewed by N specialists
who found the following issues: {merged findings summary}. Your job is to find what they
MISSED. Run `git diff origin/<base>`, and look for gaps.
...
Before classifying findings, check if any were previously skipped by the user in a prior review on this branch. If prior review data exists (e.g., in PR comments or local artifacts), use it to suppress re-reported findings that the user already intentionally skipped, provided the relevant code has not changed since.

Print: "Suppressed N findings from prior reviews (previously skipped by user)" if applicable.

Output a summary header: `Pre-Landing Review: N issues (X critical, Y informational)`

4. **Classify each finding from both the checklist pass and specialist review (Step 3.55-3.56) as AUTO-FIX or ASK.** Critical findings lean toward ASK; informational lean toward AUTO-FIX.
...
## Step 3.75: Address review comments (if PR exists)

If a PR already exists, check for any review comments that need addressing.

**If no PR exists, `gh` fails, API returns an error, or there are zero review comments:** Skip this step silently. Continue to Step 4.
...
- If user chooses A: apply the fix, commit the fixed files (`git add <fixed-files> && git commit -m "fix: address review comment — <brief description>"`).
- If user chooses C: reply explaining why it's a false positive.

**VALID BUT ALREADY FIXED:** Note what was done and the fixing commit SHA — no AskUserQuestion needed.

**FALSE POSITIVE:** Use AskUserQuestion:
- Show the comment and why you think it's wrong (file:line or [top-level] + body summary + permalink URL)
- Options:
  - A) Reply explaining the false positive (recommended if clearly wrong)
...
### Codex adversarial challenge (runs when available)
...
- Message: "It is recommended to maintain a TODOS.md organized by component, then priority (P0 at top through P4, then Completed at bottom). Would you like to create one?"
...
- Each item has a priority field with P0-P4 value
- A `## Completed` section at the bottom

**If disorganized** (missing priority fields, no component groupings, no Completed section): Use AskUserQuestion:
- Message: "TODOS.md doesn't follow a recommended structure (component groupings, P0-P4 priority, Completed section). Would you like to reorganize it?"
...
## Design Review
<If design review ran: "Design Review: N findings — M auto-fixed, K skipped.">
...
After the PR is created, check if project documentation needs updating based on the shipped changes. Review all .md files in the project, cross-reference the diff, and update anything that drifted (README, ARCHITECTURE, CONTRIBUTING, CLAUDE.md, TODOS, etc.).

Update CHANGELOG.md if present.

If any docs were updated, commit the changes and push to the same branch:
```bash
git add -A && git commit -m "docs: sync documentation with shipped changes" && git push
```
If no docs needed updating, say "Documentation is current — no updates needed."
...
If Step 8.5 created a docs commit, re-edit the PR/MR body to include the latest commit SHA in the summary. This ensures the PR body reflects the truly final state.
...
- **Never skip the pre-landing review.** If the review checklist is unreadable, use general best practices.
